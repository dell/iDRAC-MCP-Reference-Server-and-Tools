...
class CredentialDatabase:
    ...
    def add_credential(self, credential: iDRACCredential) -> bool:
        ...
        try:
            ...
            logger.info(f"[DB] Credential added/updated for system: {credential.system_id} (IP: {credential.idrac_ip}) in {self.db_path}")
            return True

        except Exception as e:
            logger.error(f"[DB] Error adding credential for system '{credential.system_id}' to {self.db_path}: {e}")
            return False
    ...
    def get_credential(self, system_id: str) -> Optional[iDRACCredential]:
        ...
        try:
            ...
            if not row:
                logger.debug(f"[DB] get_credential('{system_id}') -> NOT FOUND in {self.db_path}")
                return None
            ...
            logger.debug(f"[DB] get_credential('{system_id}') -> FOUND (IP: {row[1]}) in {self.db_path}")
            ...
            logger.error(f"[DB] Error getting credential '{system_id}' from {self.db_path}: {e}")
            return None
    ...
    def list_credentials(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.debug(f"[DB] list_credentials() -> {len(result)} system(s) from {self.db_path}")
            return result

        except Exception as e:
            logger.error(f"Error listing credentials from {self.db_path}: {e}")
            return []
